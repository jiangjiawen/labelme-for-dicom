...
class MainWindow(QtWidgets.QMainWindow, WindowMixin):
    ...
    def __init__(self, config=None, filename=None, output=None):
        ...
        windowcenter=action('WC',functools.partial(self.setWC,10),shortcuts['set_wc'],'set_wc','set wc value',enabled=True)
    ...
    def loadFile(self, filename=None):
        ...
        if QtCore.QFile.exists(label_file) and \
            ...
            self.imageData = readdicm(filename, None)
            if self.imageData is not None:
                # the filename is image not JSON
                self.imagePath = filename
            self.labelFile = None
        # image = QtGui.QImage.fromData(self.imageData)
        image=QtGui.QImage(self.imageData,self.imageData.shape[0],self.imageData.shape[1],QtGui.QImage.Format_Grayscale8)
        if image.isNull():
...
def readdicm(filename, default=None):
    try:
